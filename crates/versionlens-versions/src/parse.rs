...
pub(crate) use requirement::normalize_requirement;
pub use requirement::strip_version_prefix;

pub(crate) fn parse_version(raw: &str) -> ParsedVersion {
...
pub(crate) fn parse_padded_version(raw: &str) -> ParsedVersion {
    let version = match raw.split('.').count() {
        1 => format!("{raw}.0.0"),
        2 => format!("{raw}.0"),
        _ => raw.to_owned(),
    };
    crate::parse_semver(&version).ok()
}
