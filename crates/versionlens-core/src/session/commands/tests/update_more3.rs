use versionlens_parsers::Ecosystem::{Cpan, Haxelib, LuaRocks, Nim, Zig};
#[test]
fn apply_command_does_not_update_swift_local_package_dependency() {
    ...
    let output = session.apply_command(
        DocumentInput {
            ...
            text: package_file_fixture("apply-command-does-not-update-swift-local-package-dependency.swift"),
            workspace_root: None,
        },
        Some("update"),
        Some("LocalPackage"),
...
fn apply_command_updates_zig_github_url_tag_dependency() {
    ...
    let output = session.apply_command(
        DocumentInput {
            ...
            text: package_file_fixture("apply-command-updates-zig-github-url-tag-dependency.zig.zon"),
            workspace_root: None,
        },
        Some("update"),
        Some("known_folders"),
...
fn apply_command_does_not_update_nimble_head_dependency() {
    ...
    let output = session.apply_command(
        DocumentInput {
            uri: "file:///demo.nimble".to_owned(),
            language_id: "nim".to_owned(),
            text: package_file_fixture("apply-command-does-not-update-nimble-head-dependency.nimble"),
            workspace_root: None,
        },
        Some("update"),
        Some("foobar"),
...
fn apply_command_updates_luarocks_rockspec_dependency() {
    ...
    let output = session.apply_command(
        DocumentInput {
            ...
            text: package_file_fixture("apply-command-updates-luarocks-rockspec-dependency.0.0-1.rockspec"),
            workspace_root: None,
        },
        Some("update"),
        Some("luasocket"),
...
fn apply_command_does_not_update_luarocks_lua_runtime_dependency() {
    ...
    let output = session.apply_command(
        DocumentInput {
            uri: "file:///demo-1.0.0-1.rockspec".to_owned(),
            language_id: "lua".to_owned(),
            text: package_file_fixture("apply-command-does-not-update-luarocks-lua-runtime-dependency.0.0-1.rockspec"),
            workspace_root: None,
        },
        Some("update"),
        Some("lua"),
...
fn apply_command_does_not_update_haxelib_latest_dependency() {
    ...
    let output = session.apply_command(
        DocumentInput {
            uri: "file:///work/haxelib.json".to_owned(),
            language_id: "json".to_owned(),
            text: package_file_fixture("apply-command-does-not-update-haxelib-latest-dependency.json"),
            workspace_root: None,
        },
        Some("update"),
        Some("tink_macro"),
