...
fn resolve_package_manager(
    ...
    session_without_vulnerabilities().resolve_document_with_responses(
        DocumentInput::new(
...
#[test]
fn package_manager_pins_use_published_package_versions() {
...
#[test]
fn yarn_pins_use_the_release_catalog_for_their_version_family() {
    for (requirement, body, expected) in [
        ...
        let output = resolve_package_manager("yarn", requirement, body);
...
#[test]
fn yarn_aliases_do_not_replace_a_newer_pinned_release() {
    let output = resolve_package_manager(
        "yarn",
        "4.3.0",
...
#[test]
fn yarn_classic_sha1_pins_are_verified_and_preserved() {
    ...
    let output = resolve_package_manager("yarn", &format!("1.22.19+sha1.{current}"), body);
    assert_eq!(output.edits.len(), 1, "{output:?}");
    assert_eq!(output.edits[0].new_text, format!("1.22.22+sha1.{selected}"));
}

#[test]
fn sha224_pins_hash_the_current_and_selected_registry_artifacts() {
    let (base_url, server) =
...
#[test]
fn sha224_rejects_a_bad_current_pin_before_fetching_the_target() {
    ...
    let output =
        resolve_package_manager("pnpm", &format!("1.0.0+sha224.{}", "00".repeat(28)), body);
    assert!(output.edits.is_empty(), "{output:?}");
...
#[test]
fn sha224_artifact_fetch_failures_are_explicit() {
...
#[test]
fn package_manager_integrity_pins_are_verified_and_preserved() {
    ...
    let current = session.resolve_document_with_responses(current_input.clone(), &responses);
...
#[test]
fn package_manager_release_channels_follow_published_metadata() {
...
#[test]
fn package_manager_channels_require_a_published_target() {
    for body in [
        r#"{"versions":{"10.0.0":{}}}"#,
