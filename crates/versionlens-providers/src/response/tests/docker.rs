...
fn ignores_docker_hub_results_without_active_tag_status() {
    ...
    assert_eq!(
        latest_version_from_response(Ecosystem::Docker, "node", body),
        Some("1.0.0".to_owned())
    );
...
fn filters_docker_dotted_prerelease_tags() {
    ...
    assert_eq!(
        latest_version_from_response_with_prereleases(Ecosystem::Docker, "node", body, true),
...
fn reads_docker_registry_v2_tag_lists() {
    ...
    assert_eq!(
        latest_version_from_response(Ecosystem::Docker, "ghcr.io/org/app", body),
        Some("1.4.0".to_owned())
    );
