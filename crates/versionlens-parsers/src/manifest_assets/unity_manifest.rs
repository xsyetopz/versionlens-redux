...
pub(crate) fn parse_unity_project_manifest_json_with_paths(
    ...
    if !support::dependency_path_is_selected(dependency_paths, "dependencies") {
        return vec![];
    }
...
fn dependency_from_property(
    ...
    let requirement_start =
        support::string_content_start(requirement.range.start, requirement.range.end);
    let requirement_end =
        support::string_content_end(requirement.range.start, requirement.range.end);
...
fn property_name_range(prop: UnityJsonProperty<'_>) -> (usize, usize) {
    match &prop.name {
        ObjectPropString(lit) => (
            support::string_content_start(lit.range.start, lit.range.end),
            support::string_content_end(lit.range.start, lit.range.end),
        ),
