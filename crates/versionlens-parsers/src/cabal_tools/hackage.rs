...
fn field_value(text: &str, field: &str) -> Option<FieldValue> {
    let (line_start, line, value, value_offset) = support::field_line(text, field, true)?;
    let value_start = line_start + value_offset + value.len() - value.trim_start().len();
...
fn cabal_field_value_spans(text: &str, field: &str) -> TextSpans {
    ...
    let lines = support::line_offsets(text).collect::<Vec<_>>();
    let mut spans = vec![];
...
fn first_cabal_constraint(text: &str, start: usize, end: usize) -> Option<CabalConstraint> {
    let constraint = support::first_constraint(
        text,
        start,
        end,
        &["^>=", ">=", "<=", "==", ">", "<", "="],
    ...
    Some(CabalConstraint {
        requirement: format!(
            "{} {}",
            constraint.operator,
            text.get(constraint.version_start..constraint.version_end)?
        ),
        range_start: constraint.operator_start,
        range_end: constraint.range_end,
        prefix: text
            .get(constraint.operator_start..constraint.version_start)?
            .to_owned(),
    })
}
...
fn parse_stack_extra_dep(text: &str, value: &str, value_start: usize) -> ParsedHackageDependency {
    ...
    for prefix in ["git:", "github:"] {
        if let Some(url) = value.strip_prefix(prefix).map(str::trim) {
