...
impl RebarDependencyBlock {
    fn append_line(&mut self, line: &str) {
        support::append_source_line(&mut self.text, line);
        self.depth += rebar_brace_delta(line);
    }
}
...
fn parse_rebar_dependency_source(
    ...
    Some(support::sourced_dependency(support::SourcedDependency {
        ...
        requirement_range: requirement_offset..requirement_offset + requirement.len(),
    }))
}
...
fn rebar_dependency_tuple_spans(line: &str) -> Vec<(usize, usize)> {
    let scan_start = rebar_dependency_section_list_start(line).unwrap_or(0);
    support::balanced_brace_spans(line, scan_start, false)
}
...
fn rebar_source_url<'a>(tuple: &'a str, source: &str) -> Option<&'a str> {
    ...
    support::quoted_value_after(tuple, &marker)
}
