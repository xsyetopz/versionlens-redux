...
pub(crate) fn parse_dune_project(text: &str) -> Vec<Dependency> {
    ...
    for package in package_stanzas(text) {
        ...
        if let (Some(name), Some(version)) = (
            ...
            dependencies.push(opam::version_dependency(
                name.value.as_str(),
                version.value.as_str(),
                "version",
                offset_range(text, version.open, version.close + 1),
    ...
    while cursor < end {
        ...
        if head_symbol(text, open, close) == Some(name) {
            stanzas.push(ListStanza {
                body_start: symbol_end(
                    text,
                    support::skip_ascii_whitespace_until(text, open + 1, close),
                    close,
                ),
                close,
            });
...
fn child_atom_value(text: &str, start: usize, end: usize, name: &str) -> Option<AtomValue> {
    ...
    while cursor < end {
        ...
        if head_symbol(text, open, close) == Some(name) {
            let value_start = support::skip_ascii_whitespace_until(
                text,
                symbol_end(
                    text,
                    support::skip_ascii_whitespace_until(text, open + 1, close),
...
fn depends_entries(text: &str, start: usize, end: usize) -> Vec<Dependency> {
    ...
    let mut cursor = start;
    while cursor < end {
        cursor = support::skip_ascii_whitespace_until(text, cursor, end);
        if cursor >= end {
            break;
        }
...
fn dependency_list_entry(text: &str, open: usize, close: usize) -> Option<Dependency> {
    let name_start = support::skip_ascii_whitespace_until(text, open + 1, close);
...
fn first_dune_version_constraint(text: &str, start: usize, end: usize) -> Option<DuneConstraint> {
    ...
    while cursor < end {
        ...
        let close = matching_paren(text, open, end)?;
        let op_start = support::skip_ascii_whitespace_until(text, open + 1, close);
        if let Some((operator, after_operator)) =
            support::operator_at(text, op_start, close, &[">=", "<=", "!=", "=", "<", ">"])
        {
...
fn head_symbol<'a>(text: &'a str, open: usize, close: usize) -> Option<&'a str> {
    let start = support::skip_ascii_whitespace_until(text, open + 1, close);
